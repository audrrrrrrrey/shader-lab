Shader "shader lab/week 9/tree" {
    Properties {
        ...
        //level of detail
        _mipSteps ("mip steps", Range(0,8)) = 1
    }
    SubShader {
        ...
        Pass {
            ...
            Interpolators vert (MeshData v) {
                ...
                float height = _displacementMap.SampleLevel(sampler_displacementMap, o.uv, 0).r;
            ...
            float4 frag (Interpolators i) : SV_Target {
                ...
                // blinn phong
                float3 surfaceColor = _albedo.Sample(sampler_albedo, uv).rgb;
                surfaceColor = surfaceColor * 0.6;
                surfaceColor *= _baseColor.xyz;
                ...
                float mip = (1 - _gloss) * _mipSteps;

                float diffuseFalloff = max(0, dot(normal, light.direction));
                float specularFalloff = max(0, dot(normal, halfDirection));
                float3 indirectSpecular = SAMPLE_TEXTURE2D_LOD(_IBL, sampler_IBL, viewDirection, mip) * _gloss;

                float3 directSpecular = pow(specularFalloff, _gloss * MAX_SPECULAR_POWER + 1) * _gloss * light.color;
                float3 specular = directSpecular + indirectSpecular * _gloss;
                float3 diffuse = diffuseFalloff * surfaceColor * light.color;

                float3 finalColor = lerp(surfaceColor, diffuse, _opacity) + specular;
